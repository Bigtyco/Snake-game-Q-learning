...
class GameWorld:
  ...
  def __init__(self, width: int, height: int):
    ...
    self._screen_size.x = (self._size.x) * \
        self._block_size + 2 * self._margin_size
    self._screen_size.y = (self._size.y) * \
        self._block_size + 2 * self._margin_size + 60
  ...
  def do_paint(self, surface: Surface, highscore: int):
    ...
    img_text: Surface = pygame.font.SysFont(
        'Consolas', 18, True).render(
        score, True, (0, 0, 0))
